...
export default function UserDesignPicker() {
  ...
  useEffect(() => {
    ...
    if (balance >= cost) return; // affordable, no redirect
    // Start 5-second auto-redirect
    setInsufficientRedirectTimer(5);
    const interval = setInterval(() => {
      setInsufficientRedirectTimer((t) => {
        if (t <= 1) {
          clearInterval(interval);
          navigate(`/user/buy-credits?required=${cost}&design=${previewCard.id}`);
          return 0;
        }
        return t - 1;
      });
  ...
  const confirmUseDesign = (card) => {
    setPreviewCard(null);
    navigate(`/user/create-invitation/${card.themeId}/${card.event}/${card.id}`);
  };

  const goBuyCredits = (card) => {
    const cost = pricing[card.id]?.credits ?? 1;
    setPreviewCard(null);
    navigate(`/user/buy-credits?required=${cost}&design=${card.id}`);
  };
      ...
      {theme !== 'all' && bundles[theme] && (
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => navigate(`/user/buy-credits?required=${bundles[theme].credits || 0}&theme=${theme}`)}
              className="lux-btn"
