  ...
  return (
    <>
      ...
      >
        ...
        {/* Bride / Groom / Couple Showcase — placeholders (no real photos in demo) */}
        <BrideGroomCoupleShowcase
          ...
          groomPhoto=""
          couplePhoto=""
          brideAbout={`${bride} is one half of our story — a soul shaped by music, books and quiet kindness. Their photo will appear here on your real invitation.`}
          groomAbout={`${groom} is the other half — steady, curious and always the first to laugh. Their photo will appear here on your real invitation.`}
            ...
            <div className="px-6 md:px-16 max-w-5xl mx-auto mb-10">
              <ScrollSection>
                ...
                <h2 className="font-display text-[2.4rem] md:text-[3.4rem] leading-[1.05]" style={{ color: '#FFF8DC' }}>
                  {resolvedPerEvent.length === 1 ? 'One day of' : `${resolvedPerEvent.length} days of`}{' '}
                    ...
                    >
                      {r.design && r.theme ? (
                        ...
                        <div className="absolute inset-0 grid place-items-center" style={{ color: '#FFF8DC' }}>
                          <span className="font-display text-3xl">{evt.title}</span>
                        </div>
                      )}
                      ...
                      >
                        <div className="max-w-5xl mx-auto flex items-end justify-between flex-wrap gap-3">
                          <div>
                            <div className="lux-eyebrow text-[10px] mb-1 opacity-90">◆ {r.event}</div>
                            <div className="font-display text-xl md:text-2xl">{evt.title}</div>
                          </div>
                          <div className="text-right text-sm md:text-[15px] opacity-90 space-y-0.5">
                            {evt.date && (
                              <div className="inline-flex items-center gap-2"><Calendar className="w-3.5 h-3.5" /> {evt.date}</div>
                            )}
                            {evt.venue && (
                              <div className="flex items-center justify-end gap-2"><MapPin className="w-3.5 h-3.5" /> {evt.venue}</div>
                            )}
                          </div>
                        </div>
                        {evt.desc && (
                          <p className="mt-2 max-w-3xl text-sm leading-relaxed opacity-80">{evt.desc}</p>
                        )}
                      </div>
                    </div>
                  </ScrollSection>
