...
import ThemeOrchestrator from '@/themes/ThemeOrchestrator';
  ...
  const InvitationBody = (
    ...
    >
      ...
      {/* Top toolbar (preview only) */}
        ...
        {/* CEREMONIES ─────────────────────────────────────────────────── */}
        <Section>
          <Eyebrow color="var(--lux-gold)">◆ Ceremonies</Eyebrow>
          <h2 className="text-[2rem] md:text-[3rem] mb-10" style={{ color: '#FFF8DC', fontFamily: 'var(--theme-heading)' }}>
            <RevealWords text={`${sample.events.length} days of celebration`} themeId={themeId} />
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6" data-testid="events-grid">
            {sample.events.map((evt, i) => {
              const r = eventDesigns[i];
              return (
              <Reveal key={evt.title} kind="cardLift" themeId={themeId} delay={i * 0.08}>
                <div className="lux-glass p-5">
                  ...
                  <div className="lux-eyebrow text-[10px] mb-2" style={{ color: 'var(--lux-gold)' }}>◆ Event {String(i + 1).padStart(2,'0')}</div>
                  <h3 className="text-2xl mb-2" style={{ color: '#FFF8DC', fontFamily: 'var(--theme-heading)' }}>{evt.title}</h3>
                  <p className="text-sm leading-relaxed mb-3" style={{ color: 'rgba(255,248,220,0.6)', fontFamily: 'var(--theme-body)' }}>{evt.desc}</p>
                  <div className="flex items-center gap-2 text-sm mb-1" style={{ color: 'rgba(255,248,220,0.65)' }}>
                    <Calendar className="w-3.5 h-3.5" /> {evt.date}
                  </div>
                  <div className="flex items-center gap-2 text-sm" style={{ color: 'rgba(255,248,220,0.65)' }}>
        ...
        <Section>
          ...
          <div className="lux-glass p-7 md:p-10 flex flex-col md:flex-row md:items-center gap-6">
            ...
            <button onClick={() => fireToast('Live wall opens during the wedding (preview only).')}
        ...
        <Section>
          ...
          <div className="lux-glass p-7 md:p-10 flex flex-col md:flex-row md:items-center gap-6">
            ...
            <button onClick={() => fireToast('Upload selfie to begin AI match (preview only).')}
        ...
        <Section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4" data-testid="travel-grid">
            {sample.travel.map((t, i) => (
              <Reveal key={t.name} kind={i % 2 === 0 ? 'photoSlideIn' : 'cardLift'}
                side={i % 2 === 0 ? 'left' : 'right'} themeId={themeId} delay={i * 0.06}>
                <div className="lux-glass p-5">
                  <div className="flex items-center gap-2 mb-2">
                    <Plane className="w-4 h-4" style={{ color: 'var(--lux-gold)' }} />
                    <span className="text-[10px] tracking-[0.3em] uppercase" style={{ color: 'rgba(255,248,220,0.55)' }}>{t.type}</span>
                  </div>
                  <h3 className="text-xl mb-1" style={{ color: '#FFF8DC', fontFamily: 'var(--theme-heading)' }}>{t.name}</h3>
                  <p className="text-sm" style={{ color: 'rgba(255,248,220,0.65)' }}>{t.note}</p>
                </div>
              </Reveal>
            ))}
          </div>
  ...
  return useOrchestrator ? (
    <>
      {!openingDone && (
        <ThemeOrchestrator
          themeId={themeId}
          eventType={searchParams.get('event') || 'marriage'} // Preview defaults to marriage; ?event= overrides for testing
