...
export default function PurchaseOptionsWizard() {
  ...
  const [selectedAddons, setSelectedAddons] = useState({}); // {id: true}
  const [selectedTier, setSelectedTier] = useState(null);   // tier.id
  ...
  const handleCheckout = () => {
    setError('');
    if (shortfall > 0) {
      // Send to top-up with return URL — return-url preserves the wizard state
